...
def _perform_stateless_validation(
    ...
    Uses stateless_state_transition from the specs to validate the block
    can be executed with only the provided witness data.
    ...
    """
    from ethereum.forks.osaka.blocks import StatelessInput
    from ethereum.forks.osaka.stateless_guest import stateless_state_transition

    witness = built_block.result.execution_witness
    if witness is None:
        return  # Not an Osaka+ fork or witness not generated

    # Get Engine API parameters
    fixture_payload = built_block.get_fixture_engine_new_payload()
    params = fixture_payload.params
    block_number = built_block.header.number

    # Convert witness from hex strings to bytes
    specs_witness = _convert_execution_witness_for_validation(witness)

    # Create StatelessInput and run stateless state transition
    stateless_input = StatelessInput(
        new_payload_request=params,
        witness=specs_witness,
    )
    result = stateless_state_transition(stateless_input)
